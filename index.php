...
dispatch_get('/recent/:page', function(){
    ...
    $stmt = $db->prepare("SELECT memos.*, users.username FROM memos join users on memos.user = users.id WHERE memos.is_private=0 ORDER BY memos.created_at DESC, id DESC LIMIT 100 OFFSET " . $page * 100);
    $stmt->execute();
    $memos = $stmt->fetchAll(PDO::FETCH_ASSOC);
